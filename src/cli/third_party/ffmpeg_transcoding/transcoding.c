...
int open_media(const char *in_filename, AVFormatContext **avfc)
{
    *avfc = avformat_alloc_context();
    ...
    {
        printf("failed to open input file %s", in_filename);
        return -1;
    }
...
int encode_video(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame)
{
    ...
    while (response >= 0)
    {
        ...
        output_packet->duration =
            encoder->video_avs->time_base.den / encoder->video_avs->time_base.num /
        ...
        if (response != 0)
        {
            printf("Error %d while receiving packet from decoder: %s", response,
                   av_err2str(response));
            return -1;
        }
...
int transcode_audio(StreamingContext *decoder, StreamingContext *encoder, AVPacket *input_packet,
                    AVFrame *input_frame)
{
...
int transcode_video(StreamingContext *decoder, StreamingContext *encoder, AVPacket *input_packet,
                    AVFrame *input_frame, cprimim_Context *cpc, struct SwsContext *sws_to_bgra,
                    struct SwsContext *sws_to_yuv, AVFrame *output_frame, uint8_t *buffer)
{
    ...
    while (response >= 0)
    {
        ...
        if (response >= 0)
        {
            ...
            // YUV → BGRA for cprimim input
            uint8_t *bgra_planes[1] = {buffer};
            int bgra_stride[1] = {width * 4};

            sws_scale(sws_to_bgra, (const uint8_t **)input_frame->data, input_frame->linesize, 0,
                      height, bgra_planes, bgra_stride);

            // Process with cprimim
            cprimim_set_input(cpc, buffer);
            cprimim_Image *result = cprimim_approximate(cpc);
            uint8_t *result_data = cprimim_image_data(result);

            // BGRA → YUV for encoder
            uint8_t *result_planes[1] = {result_data};
            int result_stride[1] = {width * 4};

            sws_scale(sws_to_yuv, (const uint8_t **)result_planes, result_stride, 0, height,
            ...
            if (encode_video(decoder, encoder, output_frame))
                return -1;
        }
        av_frame_unref(input_frame);
    }
...
int process_video(Args args)
{
    ...
    decoder->filename = *args.input_path;

    StreamingContext *encoder = (StreamingContext *)calloc(1, sizeof(StreamingContext));

    encoder->filename = args.output_path;

    if (!encoder->filename)
    {
        encoder->filename = "out.mp4";
    }
    ...
    int width = decoder->video_avcc->width;
    int height = decoder->video_avcc->height;

    // Allocate persistent BGRA buffer for cprimim input
    uint8_t *bgra_buffer = av_malloc(width * height * 4);

    // Allocate output frame for encoder
    AVFrame *output_frame = av_frame_alloc();
    output_frame->format = encoder->video_avcc->pix_fmt; // Probably AV_PIX_FMT_YUV420P
    output_frame->width = width;
    output_frame->height = height;
    av_frame_get_buffer(output_frame, 32);

    // sws contexts
    struct SwsContext *sws_to_bgra =
        sws_getContext(width, height, decoder->video_avcc->pix_fmt, // Input: YUV420P
                       width, height, AV_PIX_FMT_BGRA,              // Output: BGRA for cprimim
                       SWS_BILINEAR, NULL, NULL, NULL);

    struct SwsContext *sws_to_yuv =
        sws_getContext(width, height, AV_PIX_FMT_BGRA,              // Input: BGRA from cprimim
                       width, height, encoder->video_avcc->pix_fmt, // Output: YUV420P for encoder
                       SWS_BILINEAR, NULL, NULL, NULL);
    cprimim_Context *cpc = cprimim_create_context(
        ...
        width, height, *args.background_shapes, *args.alpha);
    while (av_read_frame(decoder->avfc, input_packet) >= 0)
    {
        cprimim_reset_context(cpc);
        if (decoder->avfc->streams[input_packet->stream_index]->codecpar->codec_type ==
            AVMEDIA_TYPE_VIDEO)
        {
            if (!sp.copy_video)
            {
                if (transcode_video(decoder, encoder, input_packet, input_frame, cpc, sws_to_bgra,
                                    sws_to_yuv, output_frame, bgra_buffer))
                    return -1;
    ...
    sws_freeContext(sws_to_bgra);
    sws_freeContext(sws_to_yuv);
    cprimim_destroy_context(cpc);
    if (!(encoder->avfc->oformat->flags & AVFMT_NOFILE))
